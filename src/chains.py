...
документов информацию и предоставляй ее пользователю в виде пошаговой инструкции. Если упоминаешь документ напиши его название и его основные тезисы. Не делай 
повторов предложений. Отвечай на вопрос подробно. Отвечай так, чтобы пользователь мог самостоятельно решить свой вопрос, используя, приложение, документацию или сайт 
компании, без обращений в техническую поддержку или другие службы компании. То чего нет в документах ты не знаешь, и ответить не можешь. В конце каждого вопроса 
добавляй от себя в компании ПЭК.
'''
=======
system = """
...
пользователю обращаться в техническую поддержку, ты и есть часть технической поддержки. То чего нет в базе знаний компании ты не знаешь, и ответить не можешь.
"""
>>>>>>> Stashed changes
...
llm = LlamaCpp(
    ...
    temperature=0.05,  # Температура для управления степенью случайности в ответах
